...
class AjaxController < ApplicationController
  ...
  def check_status_update
    ...
    existing_latest_status = Status.not_deleted.get_latest_status(1).owned_by_current_user(@current_user.id)
  ...
  def read_more
    ...
    when 'home_timeline'
      @statuses = Status.not_deleted.force_index(:idx_u_on_statuses).owned_by_friend_of(@current_user.id).get_older_status_by_tweet_id(@oldest_tweet_id,request_fetch_num)
    when 'public_timeline'
      @statuses = Status.not_deleted.get_older_status_by_tweet_id(@oldest_tweet_id,request_fetch_num)
    end
  ...
  def switch_term
    ...
    when 'home_timeline'
      if date
        @statuses = Status.not_deleted.get_status_in_date(date,fetch_num).owned_by_friend_of(@current_user.id)
      else
        @statuses = Status.not_deleted.use_index(:idx_u_tcar_sisr_on_statuses).get_latest_status(fetch_num).owned_by_friend_of(@current_user.id)
      end
      if @statuses.present?
        older_status = Status.not_deleted.force_index(:idx_u_on_statuses).owned_by_friend_of(@current_user.id).get_older_status_by_tweet_id( @statuses.last.status_id_str,1 )
        @has_next = older_status.length > 0
      end
    when 'public_timeline'
        if date
          @statuses = Status.not_deleted.get_status_in_date(date,fetch_num)
        else
          @statuses = Status.not_deleted.get_latest_status(fetch_num)
        end

      if @statuses.present?
        older_status = Status.not_deleted.get_older_status_by_tweet_id( @statuses.last.status_id_str,1 )
        @has_next = older_status.length > 0
      end
