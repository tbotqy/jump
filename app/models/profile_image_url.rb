...
class ProfileImageUrl < ActiveRecord::Base
  ...
  def self.sync_record_with_status_broken
    ...
    Status::Broken.unsolved.each do |broken_status|
      ...
      unless self.where(:screen_name => screen_name).exists?
        self.create(
          :twitter_id => nil,
    ...
    records_to_fill.select(:screen_name).pluck(:screen_name).each_slice 100 do |dest_screen_names|
      ...
      begin
        ...
        twitter_users.each do |user_twitter|
          # save urls
          self.find_by_screen_name(user_twitter.screen_name).update_attributes(
  ...
  def self.apply_fresh_url_to_statuses
    # 仮 only for retweet
    fresh_records = self.valid_url
    ...
    fresh_records.each do |fresh_record|
      ...
      self.valid_url.where(:screen_name).update_all(:applied_to_status => true)
      progress_bar.increment
    end
    puts "Done."
  end
