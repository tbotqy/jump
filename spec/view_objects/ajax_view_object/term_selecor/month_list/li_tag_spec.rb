...
describe AjaxViewObject::TermSelector::MonthList::LiTag do
  let(:li_tag) do
  ...
  describe "#data_date_value" do
    ...
    it{is_expected.to eq target_month.strftime("%Y-%-m")}
  end

  describe "#data_complete_text_value" do
    subject{li_tag.data_complete_text_value}
    it{is_expected.to eq target_month.month}
  end
  ...
  describe "#anchor_href" do
    ...
    it{is_expected.to eq "#{link_path_without_date}/#{target_month.strftime("%Y-%-m")}"}
  end
end
