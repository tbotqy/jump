...
RSpec.describe DestroyUserJob, type: :job do
  ...
  shared_examples "makes no change on user's resources" do
    ...
    it { expect { subject.call rescue nil }.not_to change { TweetImportJobProgress.where(user_id: user_id).count }.from(initial_user_tweet_import_job_progress_count) }
  end
  ...
  let!(:user_tweet_import_job_progress_ids) { create_list(:tweet_import_job_progress, 3, user: user).pluck(:id) }
  ...
  let!(:initial_user_tweet_import_job_progress_count) { user_tweet_import_job_progress_ids.count }
    ...
    context "the user with given id found" do
      ...
      context "succeeded to destroy" do
        describe "destroys the user and all of its resources" do
          ...
          it { is_expected.to change { TweetImportJobProgress.exists?(user_id: user_id) }.from(true).to(false) }
        end
      end
    end
  end
